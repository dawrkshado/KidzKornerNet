...
function ParentsChildRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post("/api/child_register/", {
    ...
    <>
      ...
      >
        <div className="bg-amber-300 p-10 rounded-2xl shadow-lg flex flex-col items-center justify-center gap-4 w-[400px]">
          ...
          {showMessage === true && (
            <div className="absolute z-10 bg-green-400 opacity-90 rounded-2xl h-20 w-70 flex items-center justify-center">
              <p className="mt-4 text-lg font-semibold ">{message}</p>
            </div>
          )}

          {showMessage === false && (
            <div className="absolute z-10 bg-red-500 rounded-2xl h-20 w-70 flex items-center justify-center">
              <p className="mt-4 text-lg font-semibold ">{message}</p>
            </div>
          )}
        </div>
